...
import { getLikePersona, createUniquePersona, updateUniquePersona } from '../../../services/Persona';
...
function NuevoPersona({ token }) {
    ...
    const btnClear = async (e) => {
        ...
        form.setFieldValue('fnacimiento', '');
    }
    ...
    const create = async (e) => {
        if (fnacimiento.toString() === '0000-00-00') { message.error('Fecha de nacimiento invalida'); return; }
        let savePersona = {
            //idpersona:idpersona,
            nombre: nombre,
            apellido: apellido,
            ...
            if (rsper.body.length !== 0) {
                rsper.body.map(async (per) => {
                    if (per.documento === documento) {
                        ...
                        await updateUniquePersona({ token: token, param: savePersona.idpersona, json: savePersona }).then((rsper) => {
                            console.log(rsper);
                        }).then(async (_) => {
                            navigate(`/personas`);
                ...
                await createUniquePersona({ token: token, json: savePersona }).then(async (persona) => {
    ...
    return (
        <div >
            ...
            <Form
                ...
                <Form.Item label='Correo' id='correo' name="correo" type="mail" rules={[{ required: true, message: 'Cargue correo', },]}>
                    <Input placeholder='Correo' value={correo} onChange={(e) => setCorreo(e.target.value)} />
                </Form.Item>
                <Form.Item label='Telefono' id='telefono' name="telefono" rules={[{ required: true, message: 'Cargue telefono', },]}>
                    <Input placeholder='Telefono' value={telefono} onChange={(e) => setTelefono(e.target.value)} />
                </Form.Item>
                <Form.Item label='Dirección' id='direccion' name="direccion" rules={[{ required: true, message: 'Cargue direccion', },]}>
                    <Input placeholder='Direccion' value={direccion} onChange={(e) => setDireccion(e.target.value)} />
                </Form.Item>
                <Form.Item label='Fecha de nacimiento' id='fnacimiento' name="fnacimiento" rules={[{ required: true, message: 'Cargue fecha de nacimiento', },]} >
                    {fnacimiento ?
                        <Input id='fnacimiento' name="fnacimiento" disabled value={fnacimiento} />
                        : null}
                    <DatePicker
                        format="YYYY-MM-DD"
                ...
                <Form.Item label='Sexo' id='sexo' name="sexo" rules={[{ required: true, message: 'Seleccione sexo', },]}>
                    <Radio.Group
                ...
                <Form.Item label='Tipo documento' id='tipod' name="tipod" rules={[{ required: true, message: 'Seleccione tipo documento', },]}>
                    <Radio.Group
