...
const Header = ({ ciudades, dato, clearDato, setDato, categorias, usuario, tipoPropiedad, t }) => {
    ...
    const onSearch = (e) => {
        ...
        if (desde !== 0 && desde) setDato(dato.filter((dato) => dato.precio >= desde));
        if (hasta !== 0 && hasta) setDato(dato.filter((dato) => dato.precio <= hasta));
    }
    ...
    const miCatalogo = () => {
        ...
        if (usuario?.body?.nivel === 2) {
            return <a style={{ textDecoration: `none` }} onClick={() => navegacion('/micatalogo')} >Mi catalogo</a>
        } else {
            return null
        }
    }
        ...
        <div >
            <nav className="navbar navbar-expand-lg navbar-dark shadow"
                ...
                <div className="container d-flex justify-content-between align-items-center">
                    <div >
                        <img style={{ width: `8rem` }} onClick={() => navegacion('/home')} src={require('../componentes/img/logo1.jpeg')} alt="Logo" />
                    </div>
